...
var hideAlert = function hideAlert() {
  var el = document.querySelector('.alert'); // เลือกที่ body แล้วลบ el
...
var showAlert = function showAlert(type, msg) {
  hideAlert();
  var markup = "<div class=\"alert alert--".concat(type, "\">").concat(msg, "</div>"); // แสดงผลหลัง body tag

  document.querySelector('body').insertAdjacentHTML('afterbegin', markup); // disapear after 5 second
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 3:
            ...
            if (res.data.status === 'success') {
              (0, _alerts.showAlert)('success', 'Logged out'); // re-load page เพื่อให้หน้าที่ถูก login เปลี่ยนไป
...
// Stripe Libraries
var stripe = Stripe('pk_test_51HhyOFIN9xs379EEfNlzJxqZImj3toXjDq9kOCLCBwJbj3x7PmebPuYpGRXI3SCX4kFYe0FSjVanJvND3Y9vVCzn00KT3ZSA8Y');
...
var displayMap = function displayMap(locations) {
  // mapbox token
  mapboxgl.accessToken = 'pk.eyJ1IjoiYXJpZ2F0b3BpeCIsImEiOiJjandjNGY3aGkwbmR3NDBtZDMwMjhzb2FqIn0.G1gjYuSgO3XUy-vzX4qjnQ';
  var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/arigatopix/ckgdgfciv1pf419peiaxn7y7t',
    scrollZoom: false // center: [-118.47549, 33.987367], // [long, lat]
  ...
  locations.map(function (loc) {
    ...
    .addTo(map); // Create Popup

    new mapboxgl.Popup({
      offset: 30 // ขยับออกจากจุด location

    }).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).setLngLat(loc.coordinates).addTo(map); // Extend map bounds to include current location
...
if (loginForm) {
  loginForm.addEventListener('submit', function (e) {
    ...
    var password = document.getElementById('password').value; // login
...
if (userDataForm) {
  userDataForm.addEventListener('submit', function (e) {
    ...
    form.append('photo', document.getElementById('photo').files[0]); // updateData
    ...
    var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(e) {
      ...
      return regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              e.preventDefault(); // เปลี่ยนปุ่มให้แสดงผลว่ากำลัง update

              document.querySelector('.btn--save-password').textContent = 'Updating...';
              currentPassword = document.getElementById('password-current').value;
              password = document.getElementById('password').value;
              passwordConfirm = document.getElementById('password-confirm').value; // updat password
